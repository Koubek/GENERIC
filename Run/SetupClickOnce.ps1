...
# Copy the relevant WinClient files to the Deployment\ApplicationFiles folder
$clickOnceApplicationFilesDirectory = Join-Path $ClickOnceDirectory 'Deployment\ApplicationFiles'
. (Get-MyFilePath "SetupClickOnceDirectory.ps1") -WinClientDirectory $roleTailoredClientFolder -ClickOnceApplicationFilesDirectory $clickOnceApplicationFilesDirectory

# Save the ClientUserSettings.config file
